...
/**
 * The starting document: a brick-built survey rover assembled entirely from
 * real catalog parts at exact LDU transforms.
 *
 * Every vertical position is derived from the part's own compiled connectors
 * rather than a nominal brick height, so slopes, curved elements and the
 * windscreen land on the same planes a physical build would use.
 */
