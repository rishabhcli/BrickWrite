...
import { applyMat3, multiplyMat3, transposeMat3, type Mat3, type RigidTransform } from './math'
import type { ModelDocument, Vec3 } from './types'
 ...
 *
 * So this module mirrors poses and reports which parts a reflection cannot
 * carry faithfully. The determinant of every basis it emits stays positive: a
 * mirrored model must be buildable, so the reflected copy is placed with a real
 * rotation of a real part, or the caller is told it is not a true reflection.
 *
 * **Why this lives in `src/cad/` and not in `src/refinement/`.** It began as a
 * refinement-search helper, and for a while the editor's own Mirror command had
 * a *second*, private implementation — X-only, and composing the reflection on
 * the wrong side, so a brick standing a quarter-turn off axis came back rotated
 * rather than reflected. The correct arithmetic and the chirality test are
 * kernel geometry, not search heuristics; keeping one copy here is what stops
 * the shipped command and the search from disagreeing about what a mirror is.
 */

/** Index of the reflected coordinate: 0 = X, 1 = Y (vertical), 2 = Z. */
...
const WORLD_AXIS: Record<MirrorAxis, Vec3> = {
  0: [1, 0, 0],
  1: [0, 1, 0],
  2: [0, 0, 1],
}
 ...
 *
 * Both multiplications matter. `M · B` alone is the true reflection and carries
 * a negative determinant, which renders but may name no purchasable part; `B ·
 * M` reflects in the part's *own* frame, which is a different pose entirely for
 * any part not already axis-aligned with the mirror.
 */
 ...
 * question without a mesh. The limit is worth naming — a part whose only
 * asymmetry is unmodelled mould detail, a printed face or a door rail carried as
 * no connector at all, reads as symmetric here. This test finds the chirality
 * that shows in how a part connects, which is the chirality that matters for
 * every structural part, and it is a warning rather than a refusal because of
...
function localMirrorSafety(definitionId: string): [boolean, boolean, boolean] {
  ...
  const safeOn = (axis: MirrorAxis): boolean => {
...
export function canMirror(document: ModelDocument, partId: string, axis: MirrorAxis): boolean {
  ...
  const local = applyMat3(transposeMat3(part.transform.basis), WORLD_AXIS[axis])
  let dominant = 0
  ...
  return localMirrorSafety(part.definitionId)[dominant as MirrorAxis]
}
