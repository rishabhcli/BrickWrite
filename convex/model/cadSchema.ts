...
export const actor = z.enum(['human', 'agent'])
...
export const part = modulePart.extend({
  id,
  subassemblyId: id,
  stepId: id,
  provenance: actor,
...
export const subassembly = z.object({
  id,
  name: z.string(),
  partIds: z.array(id),
  locked: z.boolean(),
...
export const unique = <T extends { id: string }>(entry: z.ZodType<T>) =>
  z.array(entry).refine((rows) => new Set(rows.map((row) => row.id)).size === rows.length, 'Entity ids must be unique.')
export const timestamp = z.string().refine((value) => Number.isFinite(Date.parse(value)), 'Expected a timestamp.')
export const step = z.object({ id, index: revision, name: z.string(), partIds: z.array(id) })
...
export const documentShape: z.ZodType<ModelDocument> = z.object({
  ...
  modules: unique(moduleDefinition).optional(),
})
